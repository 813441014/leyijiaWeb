    ...
    if (store.state.jssdkStatus){
        ...
        if (localStorage.getItem('openId')){
            ...
            if (localStorage.getItem('sessionId')){
                ...
                //否
                //注册 跳转绑定
                wxReady(encodeURIComponent(url.split('#')[0]),jsAPIArr,()=>{store.commit('setJSSDKstatus');next('/setPhone')},()=>{});
            }
        } else {
            //否
            // 判断是否已经授权跳转
            if (url.split('?').length===1){
                //未跳转 先跳转到微信页面
                location.href=`https://open.weixin.qq.com/connect/oauth2/authorize?appid=${wxInfo.wxAppId}&redirect_uri=https%3a%2f%2fn2cs.dingdingxiujia.com%2fcustomer%2fdist%2fgroup.html&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect`;
